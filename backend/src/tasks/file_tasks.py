...
from src.tasks.base import run_async, CelerySessionLocal
...
def scan_file_for_threats(file_id: str) -> str:
    async def run() -> str:
        async with CelerySessionLocal() as session:
            service = FileUseCases(session, FileRepository(session), CeleryTaskScheduler(), LocalStorage(Settings()))
...
def send_file_alert(file_id: str) -> str:
    async def run() -> str:
        async with CelerySessionLocal() as session:
            service = AlertUseCases(session=session, file_repo=FileRepository(session), alert_repo=AlertRepository(session))
